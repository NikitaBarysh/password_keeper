...
func (h *Handler) handleSetWebsocket(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logging.Error("err to upgrade to websocket ")
	...
	for {
		...
		if errMsg != nil {
			h.logging.Error("err to read message from websocket ")
		...
		if err != nil {
			h.logging.Error("err to unmarshal message from websocket: %v", err)
			break
		}

		switch data.Action {
		case "add":
			err = h.service.DataServiceInterface.SetData(r.Context(), currUser, data.Payload, data.EventType)
			if err != nil {
		...
		case "get":
			payloadDB, err := h.service.DataServiceInterface.GetData(r.Context(), currUser, data.EventType)
			if err != nil {
			...
			if err != nil {
				h.logging.Error("error to write data to websocket:", err)
				http.Error(w, "err to write data to websocket", http.StatusInternalServerError)
				break
			}
		case "delete":
			err = h.service.DataServiceInterface.DeleteData(r.Context(), currUser, data.EventType)
			if err != nil {
