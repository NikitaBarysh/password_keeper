...
func (r *DataRepository) SetRepData(id int, text []byte, eventType string) error {
	start := time.Now()
	defer func() {
		metrics.IncDBRequestStatus("SetRepData", time.Since(start))
	}()
...
func (r *DataRepository) GetRepData(id int, eventType string) ([]byte, error) {
	start := time.Now()
	defer func() {
		metrics.IncDBRequestStatus("GetRepData", time.Since(start))
	}()
...
func (r *DataRepository) DeleteRepData(id int, eventType string) error {
	start := time.Now()
	defer func() {
		metrics.IncDBRequestStatus("DeleteRepData", time.Since(start))
	}()
